...
void initSolveMPI() {
    ...
    if (processId == 0) {
        ...
        while (!boardsToCheck.empty() && boardsToCheck.size() < INIT_POSSIBILITIES_COUNT) {
            generatePossibilitiesNextCell(boardsToCheck, possibleSolutions);
        }
        // if any solution have been found during init, add it to the finalSolutions
        for (auto const sol: possibleSolutions) {
            finalSolutions.push_back(sol);
            possibleSolutions.pop_front();
        }
    ...
    if (processId == 0) {
        ...
        while (!possibleSolutions.empty()) {
            for (int workerId = 1; workerId < countProcess; ++workerId) {
                ...
                if (idleResponse) {
                    std::cout << "[" << processId << "]: sending 1 board to process[" << workerId << "]" << std::endl;
                    sendAndConsumeDeque(possibleSolutions, workerId, CUSTOM_MPI_POSSIBILITIES_TAG, MPI_COMM_WORLD, 1);
        ...
        std::cout << "[" << processId << "]: all possibilities have been computed!" << std::endl;
        ...
        for (int workerId = 1; workerId < countProcess; ++workerId) {
            ...
            sendAndConsumeDeque(possibleSolutions, workerId, CUSTOM_MPI_POSSIBILITIES_TAG, MPI_COMM_WORLD, 0);
        }
    } else {
        ...
        do {
            ...
            unsigned int bSize = possibleSolutions.size();
            receivePushBackDeque(possibleSolutions, 0, CUSTOM_MPI_POSSIBILITIES_TAG, MPI_COMM_WORLD);
            unsigned int countReceivedSolutions = possibleSolutions.size() - bSize;
            processLoad += countReceivedSolutions;
        ...
        std::cout << "[" << processId << "]: finished to work. " << finalSolutions.size() << " solutions found over "
                  << processLoad << " possible solutions assigned." << std::endl;
    }

    // collect results
    if (processId == 0) {
        for (int i = 0; i < countProcess; ++i) {
            receivePushBackDeque(finalSolutions, i, CUSTOM_MPI_SOLUTIONS_TAG, MPI_COMM_WORLD);
            std::cout << "[" << processId << "]: resulsts from process [ " << i << "] have been collected."
                      << std::endl;
        }
        std::cout << "[" << processId << "]: all results collected : " << finalSolutions.size() << "solutions found!"
                  << std::endl;
    } else {
        // send results to master
        sendAndConsumeDeque(finalSolutions, 0, CUSTOM_MPI_SOLUTIONS_TAG, MPI_COMM_WORLD);
    }
}
